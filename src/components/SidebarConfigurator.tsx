...
// === Types (same as website ProductConfigurator) ===

interface TermInfo { slug: string; name: string; description: string; thumbnail_url: string; }
...
// === Pricing helpers (identical to live site) ===
...
function getInterpolatedPriceWithAddons(
  ...
  const combinedTiers = conditionalPrices.map((cp) => {
    ...
    for (const addon of addons) {
      if (selectedAddons[addon.id]) addonPrice += getAddonPriceAtTierQty(addon, selectedAddons[addon.id], tierQty);
    }
  ...
  if (below && above && above.qty !== below.qty) {
    return below.price + ((above.price - below.price) * (quantity - below.qty)) / (above.qty - below.qty);
  }
...
export default function SidebarConfigurator({ productId, productName, region, customerEmail, mode = 'quote' }: Props) {
  ...
  // Step state — matches website exactly
  ...
  const visibleAttributeKeys = useMemo(
    () => attributeKeys.filter((k) => {
      ...
      return !(a.terms.length === 1 && a.terms[0].slug === 'default');
    }),
  ...
  const getAddonExtra = (qty: number): number => {
    let t = 0;
    for (const a of visibleAddons) { if (selectedAddons[a.id]) t += getAddonPriceAtTierQty(a, selectedAddons[a.id], qty); }
    return t;
  };
  ...
  const handleSubmit = async () => {
    ...
    const lineItem = {
      ...
      total_gross: finalTotalGross,
      tax_percent: priceInfo.taxPercent,
      setup_fee: setupFeeOverride ?? 'Free',
    ...
    try {
      if (mode === 'order') {
        ...
        if (resp.ok && data.success) {
          setQuoteResult({ success: true, order_id: data.order_id, order_number: data.order_number, order_url: data.order_url, order_status: data.order_status });
        } else {
          setQuoteResult({ success: false, error: data.error || 'Failed to create order' });
        }
      } else {
        const resp = await fetch('/api/wc/quotes', {
          ...
          body: JSON.stringify({
            region,
            customer_email: customerEmail || '',
            quote_name: quoteName || productName,
            line_items: [lineItem],
            total: finalTotalGross,
            currency: config.currency_code || 'EUR',
          }),
        });
        const data = await resp.json();
        if (resp.ok && data.success) {
          setQuoteResult({ success: true, quote_id: data.quote_id, site_quote_id: data.site_quote_id, quote_url: data.quote_url, pdf_url: data.pdf_url, email_sent: data.email_sent, site_error: data.site_error });
        } else {
          setQuoteResult({ success: false, error: data.error || 'Failed to create quote' });
        }
      }
    } catch (err: any) {
      setQuoteResult({ success: false, error: err.message || 'Network error' });
    } finally {
  ...
  if (loading) {
    return (
      <div id="pearl-wc-steps-form">
        <div className="pearl-step-indicator"><h2>Loading...</h2></div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div id="pearl-wc-steps-form">
        <div className="pearl-step-indicator"><h2>Error loading configuration</h2></div>
        <p style={{ color: '#dc3545', fontSize: '14px', padding: '10px 21px' }}>{error}</p>
      </div>
    );
  }
  ...
  return (
    ...
    <div id="pearl-wc-steps-form" className="pearl-wc-steps-form">
      {/* Step indicator — exact same as website */}
              ...
              {!isExpanded && isCompleted ? (
                <>
                  <div className="kd-prod-attribute-title-wrapper">
                    <span>{stepNumber}: {attr.display_title || attrKey.replace('pa_', '').replace(/_/g, ' ')}</span>
                  </div>
      ...
      {visibleAddons.map((addon, addonIndex) => {
        ...
        const isCompleted = addon.display_type === 'multiple_choise'
              ...
              <div className="kd-step-collapse">
                ...
                {addon.display_type === 'multiple_choise' && Array.isArray(addon.options) && (() => {
                  ...
                  const handleCheckboxChange = (value: string, checked: boolean) => {
                    ...
                    if (newSelected.length > 0) setMaxVisibleStep(stepIndex + 1);
                  };
            ...
            <div className="kd-step-collapse">
              ...
              {(matchedVariation?.conditional_prices || config.variations?.[0]?.conditional_prices || [])
                ...
                .map((tier, idx, filteredTiers) => {
                  ...
                  const addonPrice = getAddonExtra(tierQty);
                ...
                <div className="kd-range-slider-container">
                  ...
                  <div className="kd-range-ticks">
                    {Array.from({ length: 11 }, (_, i) => {
                      ...
                      return (
                        <span key={i} className="kd-qty-range-price-tooltip" style={{ position: 'absolute', left: `${tickPosition}%`, transform: 'translateX(-50%)' }}>
                          {tickValue}
                        </span>
                      );
                    })}
            ...
            {quoteResult.success ? (
              mode === 'order' && quoteResult.order_id ? (
                ...
                <>
                  ...
                  {quoteResult.site_error && (
                    <div style={{ marginTop: '4px', color: '#b45309' }}>Site error: {quoteResult.site_error}</div>
                  )}
                </>
              )
